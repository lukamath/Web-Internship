...
# ===== <ManyToMany> ===== 
questions = db.Table('enrollments',
...
def add_question():
	...
	if request.method=='POST':
		...
		if not questiontype:
			...
			db.session.commit()
		questions=Question.query.all()
		return render_template('questionmanager.html', questions=questions) #I land again on newuser page if conditions are not all ok
	else:
